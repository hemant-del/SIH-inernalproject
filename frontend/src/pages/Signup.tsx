...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email || !password || !confirmPassword) { setError(t('auth.allFieldsRequired')); return; }
  ...
  if (!firebaseConfigured) {
    return (
      <div className="max-w-md mx-auto px-6 py-20 text-center">
        <UserPlus className="w-10 h-10 text-[#6B7280] mx-auto mb-4" />
        <h1 className="text-xl font-semibold text-[#171717] mb-2">{t('auth.signupTitle')}</h1>
  ...
  return (
    <div className="max-w-md mx-auto px-6 py-16">
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-semibold text-[#171717]">{t('auth.signupTitle')}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-[#171717] mb-1">{t('auth.name')}</label>
        ...
        <div>
          <label className="block text-sm font-medium text-[#171717] mb-1">{t('auth.confirmPassword')}</label>
          <div className="relative">
            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#9CA3AF]" />
            <input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t('auth.confirmPasswordPlaceholder')}
              className="w-full pl-10 pr-3 py-2.5 bg-[#F5F5F3] border border-[#E8E8E6] rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-[#171717]" />
          </div>
        </div>
      ...
      <p className="text-sm text-center text-[#6B7280] mt-6">
        {t('auth.hasAccount')} <Link to="/login" className="text-[#0D6E4F] font-medium hover:underline">{t('auth.loginLink')}</Link>
      </p>
    </div>
  );
}
